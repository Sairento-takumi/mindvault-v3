...
4. **에러는 모두 exit 0 + 빈 출력** — 절대 세션 시작 블로킹 안 함. 디버그는 `~/.claude/mindvault-v2/debug.log` 파일에만.
5. **무한 루프 방지** — 요약 블록에 시그니처 `# 지난 세션 요약 (MindVault v2)` 삽입, JSONL 파싱 시 동일 시그니처 포함 메시지 제외.
...
| S6 | README 테스트 섹션에 `cd apps/mindvault-v2` 전제 추가 | - |
...
- **Sprint 1 회귀 테스트**: 이 세션 SessionStart 훅이 여전히 `# 지난 세션 요약 (MindVault v2)` 블록을 주입하고 있음. 시스템 리마인더에 정상 표출 확인.
