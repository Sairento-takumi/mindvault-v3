...
예: 형의 메타 질문에 "MindVault v2 운영 중 (품질 양호)" 메모리가 자기충족
...
- C. `MV2_EXTRA_MEMORY_DIRS=path1:path2` 환경변수 — handoff/ 등 추가 디렉토리 indexing scope 진입.
...
**기술**: 환경변수 `MV2_EXTRA_MEMORY_DIRS` 또는 explicit registry.
...
- Sprint 13 Memory Compiler 는 opt-in (`MV2_AUTO_COMPILE=1`) 으로 시작 →
  안정화 후 default on.
