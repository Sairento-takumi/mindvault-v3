...
description: Sprint 11 build log — hook 회수 품질 개선. SNIPPET_CHARS 160→600 발췌 확장 + query keyword 매치 밀집도 sliding window broad word filter, _expand_wikilinks raw_cosine 게이트(factor 0.75) 무관 1-hop 노이즈 차단, MV2_EXTRA_MEMORY_DIRS env var으로 handoff 인덱싱 scope 추가
---
...
- `src/memory_indexer.py` — C(env var `MV2_EXTRA_MEMORY_DIRS` 추가 indexing scope)
...
- env var `MV2_EXTRA_MEMORY_DIRS=path1:path2` (Unix `PATH` 스타일 separator) 추가.
...
- 설치/적용: 형 shell rc에 `export MV2_EXTRA_MEMORY_DIRS=...` 1회. env 설정 후 첫 회는 mtime 변경 없어 hook trigger 안 됨 → `python3 ~/.claude/scripts/mindvault/memory_indexer.py` 수동 1회 실행 필요.
...
4. install.sh에 `MV2_EXTRA_MEMORY_DIRS` 안내 (현재는 형 shell rc 수동 설정 필요).
