...
- `MV3_EXTRACTOR_GEMMA_RETRIES` (default 2 = 최초 1 + retry 2 = 최대 3 호출)
...
- `MV3_EXTRACTOR_TAIL_TURNS` (default 40 → 80) — `load_tail_messages` window
- `MV3_EXTRACTOR_ALWAYS_FIRE=1` opt-in — `has_trigger` 결과 무시, Gemma 항상 호출
...
# = MV3_EXTRACTOR_ALWAYS_FIRE=1 + TAIL_TURNS=120 + GEMMA_RETRIES=3
...
20% — opt-in `MV3_EXTRACTOR_ALWAYS_FIRE=1` 영구화로 끌어올릴 수 있으나, 매 SessionEnd
