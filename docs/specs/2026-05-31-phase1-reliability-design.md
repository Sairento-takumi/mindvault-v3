...
- **verifier 의 역할**: 매 reverify 실행 시 각 fact 의 `current_value` 가 **여전히 라이브 코드에 존재**하는지 grep 으로 확인한다. verifier 가 fail 하면(예: 코드가 arctic 을 또 교체) **레지스트리 자체를 stale 로 큰 경고** — registry 가 조용히 거짓말하는 것을 차단(self-check). 이것이 "현행 코드와 대조"의 본질을 hardcoded 가정이 아니라 라이브 ground truth 에 묶는 핵심 요소다.
- **근거**: spec §4.2③ "현행 코드와 대조". verifier 없이는 banned-words 리스트일 뿐이고 그 리스트가 또 stale 된다. verifier 가 레지스트리를 정직하게 유지한다.
...
- **토큰 매칭 정밀도**: word-boundary + 대소문자 무시, 단 'bge' 같은 짧은 토큰의 오매칭 방지를 위해 stale_alias 는 hyphenated full token(`bge-m3`) 우선 + 단독 `bge` 는 보조. 매칭 단위·정밀도는 plan 의 테스트로 고정.
...
reverify_note: "embedding_model: 'bge-m3' 현재형 참조, 현행 arctic 미언급"
```
...
- **레지스트리 커버리지**: 초기 facts 는 {embedding model/port, summarizer}로 한정. file:line 내용 대조·임의 버전 staleness 는 비범위(§8). 즉 ③은 "열거된 canonical facts" 에 대해서만 stale 을 잡는다 — 모든 stale 을 잡는다고 약속하지 않는다.
- **판별 신호의 보수성**: §2 신호는 "현행 값 토큰을 함께 언급하면 면제"라 false-negative 가능(stale 메모리가 우연히 현행 값을 다른 맥락에서 언급하면 놓침). over-trust 도메인 특성상 false-negative 를 일부 허용(경고 누락 ≤ 잘못된 경고 남발). 보수 retrieve 필요 도메인(CDSS fork)은 신호를 완화해야 함.
